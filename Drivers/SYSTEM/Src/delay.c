...
/**
 * @brief     初始化延迟函数
 * @param     sysclk: 系统时钟频率, 即CPU频率(rcc_c_ck), 168MHz (MHz对应us级)  sysclk = 168
 * @retval    无
 */  
void delay_init(uint16_t sysclk)
{
    ...
    uint32_t reload;
#endif
    g_fac_us = sysclk;                                  /* 由于在HAL_Init中已对systick做了配置，所以这里无需重新配置 168MHZ*/
#if SYS_SUPPORT_OS                                      /* 如果需要支持OS. */
...
void delay_us(uint32_t nus)
{
    ...
    uint32_t reload = SysTick->LOAD;        /* LOAD的值-倒数至0时，重载的值 */
    ticks = nus * g_fac_us;                 /* 需要的节拍数？？？ g_fac_us = 168 */
                                            /* 1s跳168M次，1us跳168次 也就是SysTick->VAL需要变化168*/
#if SYS_SUPPORT_OS                          /* 如果需要支持OS */
    ...
    while (1)
    {
        ...
        if (tnow != told)
        {
            if (tnow < told)                /* 没有出现重载的情况 */
            {
                tcnt += told - tnow;        /* SYSTICK是一个递减的计数器 */
            }
            else                            /* 出现了一次重载，那如果出现了几次呢？ */
            {
                tcnt += told + (reload - tnow);  /* told = told减到0经过的时间; (reload - tnow) = 从reload减到tnow经过的时间 */
            }
            
            told = tnow;                    /* 先更新值再判断是否跳出循环 */
            if (tcnt >= ticks) 
            {
 ...
 * @retval      None
 */
void HAL_Delay(uint32_t Delay)                          /* weak函数 用户可重新定义 HAL_Delay只能实现毫秒级的延时，去看看定义*/
{
     delay_ms(Delay);
}
/**
 ...
 *  调用关系: tick中断 -> SysTick_Handler -> HAL_IncTick(uwTick自增)   HAL_Delay -> HAL_GetTick(得到uwTick) -> 比较前后GetTick的值和设定的Delay值
 *  中断中一般不能使用原始的HAL_Delay，因为Systick的中断优先级是最低的，中断中运行HAL_Delay会导致延时出现严重误差
*/
